...
class AudioController {
  constructor() {
    ...
    if (typeof window !== "undefined") window.__ac = this;  // DEBUG
  }

  // Called on every user gesture. Creates audio element + Web Audio graph and
  // resumes audioCtx SYNCHRONOUSLY inside the gesture. Safari does not
  // propagate the user-activation token across primer.play().then(), so
  // ctx.resume() must happen before any awaits or microtasks.
  unlock() {
    if (!this.audio) {
      this.audio = new Audio();
      this.audio.preload = "auto";
    }
    // Build the audio graph synchronously inside the gesture window so the
    // very first resume() is allowed.
  ...
  async play(url) {
    console.log('[AC] play() called with', url?.slice(0, 80), '· ctx state:', this.audioCtx?.state, '· unlocked:', this.unlocked);
    if (!this.audio) {
      this.audio = new Audio();
      this.audio.preload = "auto";
    }
    ...
    try {
      ...
      console.error('[AC] play() failed:', e.name, e.message);
    }
  }
