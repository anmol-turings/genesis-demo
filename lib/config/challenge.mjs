...
//
// The same rule as the scorecard applies, and it applies hardest here —
// a program owner reading these numbers will act on them. A row only
// carries a percentage if the build can actually record that category.
// Everything else ships `completionPercent: null` and says so on screen.

// A category is recordable when at least one metric in it has a data source.
export function isCategoryRecording(categoryId) {
  return METRIC_POOL.some(m => m.category === categoryId && m.dataAvailable !== false);
}
...
export const PROGRAM_SNAPSHOT = {
  ...
  activityByCategory: [
    ...
    { categoryId: 'focus_reset',           completionPercent: null, measurementType: 'system_recorded' },
    { categoryId: 'connection_work',       completionPercent: null, measurementType: 'system_recorded' },
  ...
  // The topics a learning report would break down. No destination in the
  // current build supplies Constellation data, so there is nothing to
  // report against them yet and no figures are shown.
  learningByTopic: [
    { topic: 'Sleep and circadian rhythm', completionPercent: null },
    { topic: 'Attention and recovery',     completionPercent: null },
    { topic: 'Boundaries at work',         completionPercent: null },
    { topic: 'Movement and energy',        completionPercent: null },
  ],
  learningDataNote:
    'No destination in the current build supplies Constellation data, so no learning completion has been recorded. These are the topics a report would break down once it does.',
};
