...
export const REALMS = [
  {
    ...
    sub: "the body's wear · the long nights · the load",
    color: "#c9a84c",
    icon: "⚕",
  ...
  },
  {
    ...
    sub: "the pulling-back · the armor · the going-quiet",
    color: "#5fa0d4",
    icon: "⊘",
  ...
  },
  {
    ...
    sub: "the heart's report · the language between body and word",
    color: "#7abcae",
    icon: "◉",
  ...
  },
  {
    ...
    sub: "the holding-up · the noticing · the staying",
    color: "#a06ac4",
    icon: "◈",
...
This realm restores: ${capability}
Profession context: ${persona.context}
${quoteContextLine(quote)}
Your bridge should NAME what is at stake for them today in the language of the realm — concise, lived, specific. Then the quote will land.
...
${quoteContextLine(quote)}
Your bridge should ACKNOWLEDGE the principle of what they did — not the literal task, the *kind* of act it was. Lead the listener to the quote as the punch.
...
export function buildQuestSkipPrompt(quest, realm, profile, quote) {
  return `TASK: The user skipped an action in realm "${realm.name}". The Shadow "${profile.shadow.name}" grew.
${quoteContextLine(quote)}
Your bridge should NAME what the Shadow gained — without shame, without plea — and leave a door open. The quote will close.

Output: 1-2 sentences only. Wrap 1-2 words in *asterisks*. End with [concept:shadow]`;
}
...
Their Shadow: ${profile.shadow.name}
${quoteContextLine(quote)}
Your bridge should NAME briefly where they are and the kind of presence you bring (not a list of services). The quote will deliver the promise.
